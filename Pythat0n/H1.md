...
data.groupby(data.Session).Score.mean()
# Query's
...
```
## maken van nieuwe dataframe
```py
# Maak een nieuw DataFrame voor België en stel de index in
covid19_be = pd.DataFrame({
    'new_casesbe': belgium_data['new_cases']
})

# Zorg ervoor dat de index correct is ingesteld
covid19_be.index = belgium_data.index
